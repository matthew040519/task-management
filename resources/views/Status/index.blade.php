          ...
          <div class="d-flex flex-wrap gap-3">
            ...
            <div class="ms-xxl-auto">
              <button type="button" data-bs-toggle="modal" data-bs-target="#verticallyCentered" class="btn btn-primary">
                <span class="fas fa-plus me-2"></span>Add Status</button>
            </div>
          </div>
        </div>
        @if(session('success'))
        ...
        <div class="modal fade" id="verticallyCentered" tabindex="-1" aria-labelledby="verticallyCenteredModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="verticallyCenteredModalLabel">Add Status</h5>
                    ...
                    <div class="d-flex align-items-center">
                        {{-- <div class="search-box">
                            ...
                            <div class="collapse mt-2" id="filterCollapse">
                                <form method="GET" action="{{ route('status.index') }}" class="d-flex align-items-center gap-2">
                                    <input type="text" name="filter_name" class="form-control" placeholder="Status Name" value="{{ request('filter_name') }}">
                                    <button type="submit" class="btn btn-primary btn-sm">Apply</button>
                                    <a href="{{ route('status.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
                                </form>
                            </div>
                    </div>
                </div>
                    ...
                    <table class="table fs-9 mb-0">
                        ...
                        <tbody class="list" id="products-table-body">
                            @if(count($status) > 0)
                                ...
                                <td class="product align-middle ps-4"><a class="fw-semibold line-clamp-3 mb-0" href="#">{{ $statusItem->status_name }}</a></td>
                                <td class="price align-middle white-space-nowrap fw-bold text-body-tertiary ps-4">{{ $statusItem->user->name }}</td>
                                <td class="align-middle white-space-nowrap text-end pe-0 ps-4 btn-reveal-trigger">
                                    <div class="btn-reveal-trigger position-static">
                                        <button class="btn btn-sm dropdown-toggle dropdown-caret-none transition-none btn-reveal fs-10" type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent"><span class="fas fa-ellipsis-h fs-10"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end py-2">
                                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#verticallyCenteredupdate{{ $statusItem->id }}">Update</button>
                                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#verticallyCentereddelete{{ $statusItem->id }}">Delete</button>
                                        </div>
                                    </div>
                                </td>
                                </tr>
                                @include('Status.delete')
